...
class ResNet(BaseModule):
    def __init__(self, config, name):
        ...
        print("%s groups: %d" \
              % (type(self), self.groups))
        self.base_width = config["width_per_group"]
        print("%s width_per_group: %d" \
              % (type(self), self.base_width))
...
if __name__ == '__main__':
    ...
    config = {"layers": [3, 4, 6, 3], "dropout": 0, "groups": 1, "width_per_group": 64, "channels": 3,}
    model = ResNet(config, "ResNet50")
